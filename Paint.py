...
class Example(Frame):
    ...
    def initUI(self):
        ...
        b20=Button(toolbar, image=line2 ,relief=FLAT,command=self.createline)
        b20.image=line2
        ...
        b16=Button(toolbar, command=self.outline,text="OUTLINE", bg="white")
        b16.pack(side=LEFT, padx=1, pady=1)
    ...
    def decr_width(self):
        ...
        if(parameter=="eraser" and er_size>5):
            er_size=er_size-5
        elif (parameter=="Line" and br_size>2):
            br_size=br_size-1
...
def b1down(event):
    ...
    if curr_object is not None and check is not 3 and resize==0:
        deselect()
...
def b1up(event):
    ...
    if (parameter=="crline" and check==1):
            ...
            draw.line([xold,yold,event.x,event.y],fill=color)
...
def motion(event):
    ...
    if b1 == "down" and parameter=="Line":
        ...
        if xold is not None and yold is not None:
            ...
            draw.line(((xold,yold),(event.x,event.y)),fill=color,width=br_size)
        xold = event.x
        yold = event.y
...
line2 = PhotoImage(file = "line3.gif")
print "smriti"
